...
**Playground (Phase 3a):** `/components` is a category-grouped index (interactive previews) linking to a page per component at `/components/[name]` (SSG via `generateStaticParams` over `catalog`). Each detail page has a live demo, prop knobs, generated usage code, and a **Loop/Scroll/Live** mode toggle (one-shot demos loop via `LoopDemo`; scroll-driven ones demo inside a `ScrollBox`). Demos live in one registry — `apps/playground/app/components/_registry/` (`{ controls, modes, render, code }` per component, `render(values, { accent, mode })`) — used by both index and detail; add a component → add a registry entry. **Token editing is folded into props:** a token-select control carries a `token` tag (duration/easing/distance/stagger) and `PropControls` shows a collapsed "value of …" expander that edits that token in a `MotionProvider tokens={…}` override (no standalone panel). `MotionProvider`'s optional `tokens` override powers it; `Parallax`/`ScrollProgress` accept a scroll `root`/`container`. The registry/`npx shadcn add` distribution is **Phase 3b**, after the component edit pass.
